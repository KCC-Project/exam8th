			...
			<div style="margin: 0px; padding-left: 20px; height:35px;">
				 ...
				 <div>
					<h3 style="color: #3c8dbc">
						...
						<center>Parents Request For Student Achievements And Reports</center>
	...
	<div class="box box-default with-border">
		<div>
			<div style="margin: 0px;  height:35px;">
				 <div>
					<h3 style="color: #3c8dbc" >
					<label class="switch pull-right" style="margin-top: -3px"> <input type="checkbox"
...
function fun(){
	...
	function loadAllParentsRequest(url, method, data){
		$('#parents_mode').DataTable({
			destroy : true,
			paging : true,
			searching : true,
			"processing" : true,
		  ...
		  $("#parents_mode").on('click','.editGrant',function(event) {
			  var table = $("#parents_mode").DataTable();
	    		...
	    		RequestStatus(data,method,url,fun,"Permission Granted");
		});
		  $("#parents_mode").on('click','.editNo',function(event) {
			  var table = $("#parents_mode").DataTable();
	    		...
	    		RequestStatus(data,method,url,fun,"Request Invalid So Request Delete Sucessfull");
