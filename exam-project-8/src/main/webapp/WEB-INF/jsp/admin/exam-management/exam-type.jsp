...
<form id="examType-edit-form" method="post" class="form-horizontal well"
		style="display: none;">
		<div class="form-group">
		...
		<div class="form-group">
			...
			<div class="col-md-9">
				<input type="text" class="form-control" name="exam_type_name" required />
			</div>
		</div>
		<div class="form-group">
		...
		function(e) {
			...
			function formToJSON() {
				var data = JSON.stringify({
					"type_name" : $('#examType-add-form').find(
							'[name="exam_type_name"]').val(),
					"status" : $('#examType-add-form').find(
...
.on(
		...
		function(e) {
			...
			function formToJSON() {
				var data = JSON.stringify({
					"exam_type_id" : $('#examType-edit-form').find(
					'[name="exam_type_id"]').val(),
					"type_name" : $('#examType-edit-form').find(
							'[name="exam_type_name"]').val(),
					"status" : $('#examType-edit-form').find(
							'[name="status"]:checked').val(),
	...
	$(document).ready(
			...
			function() {
				...
				select2Function(url1, url2, method1, method2, placeholder,
						loadExamTypeInformation);
	...
	function loadExamTypeInformation(url, method, data) {
		...
		$(".editBtns").click(
				function(event) {
					...
					$('#examType-edit-form').find(
							'[name="exam_type_id"]').val(
							data['exam_type_id']).end().find(
							'[name="exam_type_name"]').val(
							data['type_name']).end()
					$(
